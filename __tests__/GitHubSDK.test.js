...
describe("GitHubSDK class", () => {
    ...
    describe("getUserInfo(username)", () => {
        ...
        it("Should return user login when correct username is specified", async () => {
            const ghSDK = new GitHubSDK(username, token);
            const { login } = await ghSDK.getUserInfo(username);
    ...
    describe("getUserPublicRepos(username)", () => {
        ...
        it("Should return repos list when username is correct", async () => {
            const ghSDK = new GitHubSDK(username, token);
            const result = await ghSDK.getUserPublicRepos(username);
    ...
    describe("getRepoCommits(username, repoName)", () => {
        ...
        it("Should throw exception when username is not specified", async () => {
            const ghSDK = new GitHubSDK(username, token);
            await expect(ghSDK.getRepoCommits(undefined, "practice-js-testing")).rejects.toThrow(
                "No username or repo name specified!"
            );
        });

        it("Should throw exception when repo name is not specified", async () => {
            const ghSDK = new GitHubSDK(username, token);
            await expect(ghSDK.getRepoCommits("szymonsuchanowski")).rejects.toThrow(
                "No username or repo name specified!"
            );
        });
        ...
        it("Should throw exception when specified repo name is incorrect", async () => {
            const ghSDK = new GitHubSDK(username, token);
            await expect(ghSDK.getRepoCommits("szymonsuchanowski", "--repo-does-not-exist--")).rejects.toThrow(
                "Not Found"
            );
        });
    ...
    describe("getRepoReadme(username, repoName)", () => {
        ...
        it("Should throw exception when username is not specified", async () => {
            const ghSDK = new GitHubSDK(username, token);
            await expect(ghSDK.getRepoReadme(undefined, "practice-js-testing")).rejects.toThrow(
                "No username or repo name specified!"
            );
        });
        ...
        it("Should throw exception when repo name is not specified", async () => {
            const ghSDK = new GitHubSDK(username, token);
            await expect(ghSDK.getRepoReadme("szymonsuchanowski")).rejects.toThrow(
                "No username or repo name specified!"
            );
        });
        ...
        it("Should throw exception when specified repo name is incorrect", async () => {
            const ghSDK = new GitHubSDK(username, token);
            await expect(ghSDK.getRepoReadme("szymonsuchanowski", "--repo-does-not-exist--")).rejects.toThrow(
                "Not Found"
            );
        });
    ...
    describe("getRepoEvents(username, repoName)", () => {
        ...
        it("Should throw exception when username is not specified", async () => {
            const ghSDK = new GitHubSDK(username, token);
            await expect(ghSDK.getRepoEvents(undefined, "practice-js-testing")).rejects.toThrow(
                "No username or repo name specified!"
            );
        });
        ...
        it("Should throw exception when repo name is not specified", async () => {
            const ghSDK = new GitHubSDK(username, token);
            await expect(ghSDK.getRepoEvents("szymonsuchanowski")).rejects.toThrow(
                "No username or repo name specified!"
            );
        });
        ...
        it("Should throw exception when specified repo name is incorrect", async () => {
            const ghSDK = new GitHubSDK(username, token);
            await expect(ghSDK.getRepoEvents("szymonsuchanowski", "--repo-does-not-exist--")).rejects.toThrow(
                "Not Found"
            );
        });
    ...
    describe("sendInvitation(invitedUser, repoName", () => {
        ...
        it("Should throw exception when invitedUser is not specified", async () => {
            const ghSDK = new GitHubSDK(username, token);
            await expect(ghSDK.sendInvitation(undefined, "practice-js-testing")).rejects.toThrow(
                "No username or repo name specified!"
            );
        });
        ...
        it("Should throw exception when repo name is not specified", async () => {
            const ghSDK = new GitHubSDK(username, token);
            await expect(ghSDK.sendInvitation("szymonsuchanowski")).rejects.toThrow(
                "No username or repo name specified!"
            );
        });
        ...
        it("Should throw exception when specified repo name is incorrect", async () => {
            const ghSDK = new GitHubSDK(username, token);
            await expect(
                ghSDK.sendInvitation("szymonsuchanowski-test", "----repo--does---not-exi///st/////----")
    ...
    describe("removeInvitation(username, repoName)", () => {
        ...
        it("Should throw exception when username is not specified", async () => {
            const ghSDK = new GitHubSDK(username, token);
            await expect(ghSDK.removeInvitation(undefined, "practice-js-testing")).rejects.toThrow(
                "No username or repo name specified!"
            );
        });
        ...
        it("Should throw exception when repo name is not specified", async () => {
            const ghSDK = new GitHubSDK(username, token);
            await expect(ghSDK.removeInvitation("szymonsuchanowski-test")).rejects.toThrow(
                "No username or repo name specified!"
            );
        });
        ...
        it("Should throw exception when specified repo name is incorrect", async () => {
            const ghSDK = new GitHubSDK(username, token);
            await expect(
                ghSDK.removeInvitation("szymonsuchanowski-test", "---repo--does-not-exi///st----")
            ).rejects.toThrow("Not Found");
        });
    });
    ...
    describe("flow: create test repo -> try to create the same test repo -> send invitation for user to created test repo -> trying remove invitation for not invited user -> remove invitation for invited user -> delete created test repo", () => {
        ...
        it("Should throw exception when trying to remove invitation for not invited user", async () => {
            ...
            const repoTestName = "--new-test-repo--";
            expect(ghSDK.removeInvitation(userNotInvited, repoTestName)).rejects.toThrow(
                "Specified user was not invited!"
            );
        });
